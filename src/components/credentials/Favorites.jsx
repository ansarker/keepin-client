...
const Favorites = () => {
  ...
  useEffect(() => {
    const fetchPasswords = async () => {
      try {
        const response = await axios.get("/passwords/favorites", {
          headers: {
    ...
    <div className="mb-3 md:mb-6">
      <div className="mb-4 bg-gray-600 rounded-md p-4 flex items-center justify-between">
        <div className="flex rounded-xl bg-white flex-col items-center justify-center p-3">
          ...
          <div>
            {passwords.length < 1 ? (
              <span>Nothing in favorites</span>
            ) : (
      ...
      <h1 className="text-gray-700 text-lg font-bold">Favorite Passwords</h1>
      <div className="mb-4">
        ...
        {passwords.length > 0 ? (
          ...
          <NoData heading="List Empty" message="You have no favorite passwords" />
        )}
      </div>
      <h1 className="text-gray-700 text-lg font-bold">Favorite Cards</h1>
      <div className="mb-4">
        ...
        {cards.length > 0 ? (
          ...
          <NoData heading="List Empty" message="You have no favorite cards" />
        )}
      </div>
    </div>
  );
